...
/**
 * Created with IntelliJ IDEA. User: zmichaelov Date: 7/28/13 Time: 4:16 AM To
...
public class GameActivity extends ColorBlobDetectionActivity {
	...
	private void updateStats() {
		final int health = mPlayer.getHealth();
		mPlayer.fetchInBackground(new GetCallback<LazerUser>() {
			...
			public void done(LazerUser newPlayer, ParseException e) {
				...
				if (!canShoot()) {
					timeoutOut--;
					if (canShoot()) {
						mPlayer.setHealth(origHealth);
						mPlayer.saveInBackground();
					}
					return;
				}
